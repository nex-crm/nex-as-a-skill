/**
 * User Acceptance Tests (UAT) for the Nex TUI.
 *
...
async function waitForStartup(tui: TuiTest): Promise<void> {
  ...
  // or "Welcome to Nex" / "/init to get started" (without API key)
  const found = await tui.waitForMatch(
    /What would you like|Welcome to Nex|get started|Message Team-Lead/i,
    STARTUP_TIMEOUT,
  );
...
describe("UAT Acceptance Tests", () => {
  ...
  it("AC-5: /init starts onboarding flow", async () => {
    ...
    try {
      ...
      const found = await tui.waitForMatch(
        /email|set up|API key|valid|expired|provider|Welcome to Nex! Let|Enter your/i,
        ACTION_TIMEOUT,
      );
