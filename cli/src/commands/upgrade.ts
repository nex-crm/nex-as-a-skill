...
function detectPackageManager(): "bun" | "node" {
  ...
  if (existsSync(bunGlobalPath)) {
    try {
      ...
      if (out.includes(".bun")) return "bun";
    } catch { /* fall through */ }
  }
  return "node";
}
...
export async function runUpgrade(): Promise<void> {
  ...
  try {
    ...
    try { unlinkSync(join(nexDir, "last-update-check")); } catch { /* ignore */ }
    try { unlinkSync(join(nexDir, "update-snoozed")); } catch { /* ignore */ }
