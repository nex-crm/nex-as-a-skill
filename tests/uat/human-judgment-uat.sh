#!/bin/bash
# Human Judgment UAT Test Suite for nex TUI
...
assert_layout() {
  ...
  if ! echo "$bottom3" | grep -qi "type a message\|INSERT\|NORMAL\|/help\|/quit\|nex v"; then
...
cd "$ROOT" && go build -o nex ./cmd/nex 2>&1
echo "Build complete."
echo ""
...
#   "I just installed nex. What am I looking at?"
# ═══════════════════════════════════════════════════════════════════════
CURRENT_P=1
echo ""
