# Handoff Document — Nex Multi-Agent CLI

> Complete context for any agent continuing this work. Read this before touching code.

**Date:** 2026-03-24
**Branch:** `nazz/experiment/multi-agent-cli`
**Location:** `/Users/najmuzzaman/Documents/nex/nex-as-a-skill/cli/.worktrees/go-bubbletea-port`
...
1. `./nex` launches a team of AI agents that collaborate organically
...
6. Decisions get stored permanently in the Nex knowledge graph
...
├── tmux session "nex-team" (-L nex server to avoid nesting):
...
├── Nex MCP — team_broadcast/poll/status/members tools on each agent
...
**Durable (Nex Knowledge Graph):** Decisions, facts, outcomes. Persists forever. Agents use `add_context` to store, `query_context` to retrieve. Like decisions in Notion.
...
- **Agent Teams experimental feature** — only tmux/iTerm2, no custom UI. Can't integrate Nex context.
- **claude-peers-mcp** — good broker pattern but separate from Nex. Built the same thing into Nex MCP instead.

### Things That Worked
- **tmux with separate server** (`-L nex`) — avoids nesting issues when running inside Claude Code's tmux.
...
|---------|--------|-------|
| `./nex` launches team | ✅ | Default: founding team, 7 agents |
...
| `./nex kill` from outside | ✅ | Graceful cleanup |
| `./nex --solo` TUI | ✅ | Single-agent with 55+ commands |
| `./nex --cmd` dispatch | ✅ | Non-interactive |
...
```
cmd/nex/main.go              # Entry point: ./nex, ./nex kill, --solo, --cmd
cmd/nex/channel.go           # Channel TUI: polls broker, renders, input
...
1. In `launcher.go claudeCommand()`, add `--dangerously-load-development-channels nex` to the claude args
...
### To run E2E tests:
```bash
cd /Users/najmuzzaman/Documents/nex/nex-as-a-skill/cli/.worktrees/go-bubbletea-port
...
# MCP server
cd /Users/najmuzzaman/Documents/nex/nex-as-a-skill/mcp
bun run build
