...
**Source Branches:** `kill-saas` (nex-cli), `feat/multi-threaded-agents` (nex-as-a-skill), TS CLI main
...
### Table Render
Port from `nex-cli/ts/src/ui/table.ts` (100 lines).
...
### Taskboard Render
Port from `nex-cli/ts/src/ui/taskboard.ts` (218 lines).
...
### Insights Render
Port from `nex-cli/ts/src/ui/insights.ts` (70 lines).
...
### Timeline Render
Port from `nex-cli/ts/src/ui/timeline.ts` (80 lines).
...
- Persistence: `~/.nex-cli/chat/channels.json`
...
Port from `nex-cli/ts/src/ui/graph.ts` (595 lines).
