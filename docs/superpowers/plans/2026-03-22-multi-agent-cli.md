...
Run: `cd /Users/najmuzzaman/Documents/nex/nex-as-a-skill/cli/.worktrees/go-bubbletea-port && go test ./internal/agent/ -run TestPack -v`
...
- [ ] **Step 5: Add Pack and TeamLeadSlug to NexConfig**

In `internal/config/config.go`, add two fields to `NexConfig`:

```go
// Add to NexConfig struct after GeminiAPIKey:
...
func TestDefaultResolverUsesClaudeCode(t *testing.T) {
	...
	cfg := config.NexConfig{} // empty provider
...
git commit -m "feat: default LLM provider to Claude Code instead of Nex Ask"
```
...
Run: `go build -o nex ./cmd/nex && ./nex --version`
...
3. Show picker: Claude Code / Gemini / Nex Ask
...
Run: `go build -o nex ./cmd/nex && ./nex`
Type: `/init`
...
Run: `cd /Users/najmuzzaman/Documents/nex/nex-as-a-skill/cli/.worktrees/go-bubbletea-port && go test ./...`
Expected: All tests pass (existing + new)

- [ ] **Step 2: Build binary**

Run: `go build -o nex ./cmd/nex`
Expected: Clean build, binary under 15MB

- [ ] **Step 3: Verify non-interactive mode**

Run: `./nex --version && ./nex --cmd "help"`
