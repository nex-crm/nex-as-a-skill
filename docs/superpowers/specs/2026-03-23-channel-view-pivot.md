...
```
User's Terminal
├── Primary: nex TUI (Bubbletea) — the Channel
...
### tmux Integration
- On boot: `tmux new-session -d -s nex-agents` (if not already in tmux)
- For each agent: `tmux new-window -d -t nex-agents -n <slug> "claude -p <prompt>..."`
- Capture: `tmux capture-pane -p -t nex-agents:<slug>` every 500ms
