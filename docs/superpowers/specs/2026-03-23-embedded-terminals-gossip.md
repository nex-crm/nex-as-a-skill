...
Launch `nex` and see a real Claude Code terminal for each agent on your team. When the CEO thinks, the FE Engineer sees it. When FE runs a command, BE knows about it. They debate, brainstorm, correct each other — like a real team in a room. The user talks to everyone; the CEO orchestrates execution.
