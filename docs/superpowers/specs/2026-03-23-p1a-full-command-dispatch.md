...
| N8 | Non-interactive pipeline check | `nex --cmd "record list deal --limit 5"` | Top 5 deals, exits cleanly |
...
| A7 | Config path | `config path` | Prints ~/.nex/config.json path |
...
All 55+ commands also tested via `nex --cmd "<command>"` for CI coverage in `tests/e2e/commands_test.go`.
