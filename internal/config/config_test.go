...
func TestSaveCreatesParentDirs(t *testing.T) {
	withTempConfig(t, func(dir string) {
		if err := Save(NexConfig{APIKey: "k"}); err != nil {
			t.Fatalf("Save failed: %v", err)
		}
		path := filepath.Join(dir, ".nex", "config.json")
...
func TestSaveWritesValidJSON(t *testing.T) {
	withTempConfig(t, func(dir string) {
		if err := Save(NexConfig{APIKey: "k", Email: "e@e.com"}); err != nil {
			t.Fatalf("Save: %v", err)
		}
		raw, _ := os.ReadFile(filepath.Join(dir, ".nex", "config.json"))
...
func TestResolveAPIKeyConfigFile(t *testing.T) {
	withTempConfig(t, func(_ string) {
		...
		_ = Save(NexConfig{APIKey: "file-key"})
...
func TestResolveFormatConfigDefault(t *testing.T) {
	withTempConfig(t, func(_ string) {
		_ = Save(NexConfig{DefaultFormat: "json"})
...
func TestResolveTimeoutConfigDefault(t *testing.T) {
	withTempConfig(t, func(_ string) {
		_ = Save(NexConfig{DefaultTimeout: 60_000})
...
func TestPersistRegistrationMerges(t *testing.T) {
	withTempConfig(t, func(_ string) {
		_ = Save(NexConfig{APIKey: "existing-key", DefaultFormat: "json"})
...
func TestBaseURLDevURLEnv(t *testing.T) {
	t.Setenv("NEX_DEV_URL", "http://localhost:4000")
