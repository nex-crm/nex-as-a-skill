...
func (l *Launcher) buildPrompt(slug string) string {
	...
	if slug == l.pack.LeadSlug {
		...
		sb.WriteString("\n== CHANNEL RULES ==\n")
		sb.WriteString("You are in a shared team channel. All teammates can see all messages.\n")
		sb.WriteString("Use the send_message tool (from claude-peers-mcp) to post to the channel.\n")
		sb.WriteString("When you post, ALL teammates receive your message instantly.\n\n")
		sb.WriteString("YOUR ROLE AS LEADER:\n")
		sb.WriteString("1. When the user gives a directive, analyze it and share your plan with the team\n")
		sb.WriteString("2. Tag relevant specialists: @fe, @be, @pm etc.\n")
		sb.WriteString("3. Listen to team input — they may push back or suggest improvements\n")
		sb.WriteString("4. You make the FINAL decision on execution approach\n")
		sb.WriteString("5. Once decided, clearly assign tasks: '@fe build the hero section'\n\n")
		sb.WriteString("CONVERSATION STYLE:\n")
		sb.WriteString("- Be concise. This is a team chat, not an essay.\n")
		sb.WriteString("- Respond to @mentions from teammates\n")
		...
		sb.WriteString("1. Listen to the channel — respond when the topic is in your expertise\n")
		sb.WriteString("2. If @tagged by anyone, you MUST respond\n")
		sb.WriteString("3. Proactively share your perspective on relevant discussions\n")
		sb.WriteString("4. Push back if you disagree — explain why with expertise\n")
		sb.WriteString("5. When assigned a task by the leader, execute it\n")
		sb.WriteString("6. Share progress updates in the channel so the team stays informed\n\n")
		sb.WriteString("CONVERSATION STYLE:\n")
		...
		sb.WriteString("- When you finish a task, post the result to the channel\n")
	}
