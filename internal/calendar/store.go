...
// CalendarStore persists schedules and events to ~/.nex-cli/calendar.json.
...
// If path is empty, defaults to ~/.nex-cli/calendar.json.
func NewCalendarStore(path string) *CalendarStore {
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".nex-cli", "calendar.json")
	}
